...
# ==============================================================================
# AI-STYLE INSIGHTS  &  SMART ASSISTANT  (rule-based "pretend AI")
# ==============================================================================
...
def smart_assistant_response(query, df):
    """Rule-based Q&A engine that answers questions about the active dataset."""
    q = query.lower()
    types = detect_column_types(df)

    if any(w in q for w in ["hello", "hi ", "hey", "hi!"]) or q.strip() in ("hi", "hey"):
        return "Hello! I'm your dataset assistant. Ask me about rows, columns, missing values, correlations, or the target variable."
    if "row" in q or "shape" in q:
        return f"The dataset has **{df.shape[0]:,} rows** and **{df.shape[1]} columns**."
    if "column" in q and "numeric" not in q and "categorical" not in q:
        cols = ", ".join(df.columns[:15])
        more = "..." if df.shape[1] > 15 else ""
        return f"There are **{df.shape[1]} columns**: {cols}{more}"
    if "missing" in q or "null" in q or "nan" in q:
        ...
        worst = worst[worst > 0]
        return f"There are **{total_missing} missing values** in total. The most affected column is **'{worst.index[0]}'** with {int(worst.iloc[0])} missing entries."
    if "correlat" in q:
        numeric_df = df[types["numeric"]]
        ...
        return f"The strongest correlation is between **'{max_pair[0]}'** and **'{max_pair[1]}'** (|r| = {corr.loc[max_pair]:.2f})."
    if "target" in q:
        candidates = [c for c in df.columns if c.lower() in ("target", "churn", "label", "class", "y")]
        if candidates:
            return f"The likely target variable is **'{candidates[0]}'**."
        return "I couldn't auto-detect an obvious target column — you can choose one on the Machine Learning page."
    if "numeric" in q:
        return f"Numeric columns ({len(types['numeric'])}): {', '.join(types['numeric']) or 'none'}."
    if "categorical" in q:
        return f"Categorical columns ({len(types['categorical'])}): {', '.join(types['categorical']) or 'none'}."
    if "duplicate" in q:
        return f"There are **{types['duplicates']} duplicate rows** in the dataset."
    if "describe" in q or "summary" in q or "statistic" in q:
        return "Check the Dashboard and Upload Dataset pages for a full statistical summary, or ask me about a specific column's average."
    if "mean" in q or "average" in q:
        for col in types["numeric"]:
            if col.lower() in q:
                return f"The average of **'{col}'** is **{df[col].mean():.2f}**."
        return "Tell me which numeric column you'd like the average for, e.g. 'average of Age'."
    if "help" in q:
        return "Try asking: 'How many rows?', 'Any missing values?', 'Most correlated feature?', 'What's the target?', 'Numeric columns?'"
    return "I'm not sure about that yet — try asking about rows, columns, missing values, correlations, or the target variable."
...
def page_ai_insights():
    ...
    st.markdown("The AI analyst below reviews your dataset and reports what it finds.")

    cols = df.columns.tolist()
    default_target = "Churn" if "Churn" in cols else None
    options = ["None"] + cols
    default_index = options.index(default_target) if default_target else 0
    target = st.selectbox("Analyze with respect to target (optional)", options, index=default_index)
    target = None if target == "None" else target

    insights = generate_ai_insights(df, target)
    html_parts = ['<div class="chat-container">']
    for i, ins in enumerate(insights):
        html_parts.append(
            f'<div class="ai-bubble ai-{ins["type"]}" style="animation-delay:{i*0.12}s">'
            f'<span class="ai-icon">{ins["icon"]}</span><span class="ai-text">{ins["text"]}</span></div>'
        )
...
# ==============================================================================
def handle_assistant_query(query, df):
    st.session_state.chat_history.append({"role": "user", "content": query})
    response = smart_assistant_response(query, df)
    st.session_state.chat_history.append({"role": "assistant", "content": response})
    add_log(f"Assistant query: {query}", "💬")
    st.rerun()
...
def page_smart_assistant():
    ...
    st.markdown("Ask about your dataset — try *'How many rows?'* or *'Any missing values?'*")

    for msg in st.session_state.chat_history:
        with st.chat_message(msg["role"]):
            st.markdown(msg["content"])

    quick_qs = ["How many rows?", "Missing values?", "Most correlated feature?", "What's the target?"]
    quick_cols = st.columns(4)
    for i, q in enumerate(quick_qs):
        if quick_cols[i].button(q, key=f"quick_{i}"):
            handle_assistant_query(q, df)

    prompt = st.chat_input("Ask me about your dataset...")
    if prompt:
        handle_assistant_query(prompt, df)
...
def render_sidebar():
    with st.sidebar:
        ...
        pages = [
            "🏠 Dashboard", "📂 Upload Dataset", "📊 Visual Analytics", "🤖 Machine Learning",
            "📈 Feature Engineering", "🧠 AI Insights", "💬 Smart Assistant",
            "⚙ Settings", "📜 Logs", "👤 About",
        ]
